// YouTube Translation Sidebar - Background Service Worker
...
async function handleTranslate(text, from, to) {
  ...
  if (engine === 'youdao' && settings.youdaoAppKey && settings.youdaoAppSecret) {
    try {
      const result = await translateWithYoudao(text, from, to, settings);
      return { translation: result, source: 'youdao' };
    } catch (e) {
      console.warn('Youdao failed, fallback to Google:', e.message);
    }
  }
  ...
  if (engine === 'mymemory') {
    try {
      ...
      return { translation: result, source: 'mymemory' };
    } catch (e) {
      console.warn('MyMemory failed, fallback to Google:', e.message);
    }
  }

  const result = await translateWithGoogle(text, from, to);
  return { translation: result, source: 'google' };
}
...
async function translateWithMyMemory(text, from, to) {
  const langMap = { 'auto': '', 'zh-CN': 'zh-CN', 'en': 'en', 'ja': 'ja', 'ko': 'ko' };
  const sl = langMap[from] || 'en';
  const tl = langMap[to] || 'zh-CN';
  const langpair = sl ? `${sl}|${tl}` : `en|${tl}`;
...
async function translateWithYoudao(text, from, to, settings) {
  const { youdaoAppKey, youdaoAppSecret } = settings;
  if (!youdaoAppKey || !youdaoAppSecret) {
    throw new Error('Youdao API key not configured');
  }

  const salt = Date.now().toString();
  const sign = md5(youdaoAppKey + text + salt + youdaoAppSecret);
  ...
  const data = await resp.json();
  if (data.errorCode === '0') {
    return data.translation ? data.translation.join('\n') : '';
  }
  throw new Error(`Youdao error ${data.errorCode}`);
}

// MD5 for Youdao API signing
function md5(s) {
  ...
  var x = sb(s), a = 1732584193, b = -271733879, c = -1732584194, d = 271733878, i, olda, oldb, oldc, oldd;
